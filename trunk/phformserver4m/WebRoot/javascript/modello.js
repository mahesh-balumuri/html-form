...
Function.prototype.closure2 = function(obj)
{
  // Init object storage.
  if (!window.__objs2)
  {
  ...
  var objId = obj.__objId;
  if (!objId)
    __objs2[objId = obj.__objId = __objs2.length] = obj;

  // Make sure the function has an id and is stored in the function store.
  var funId = fun.__funId;
  if (!funId)
    __funs2[funId = fun.__funId = __funs2.length] = fun;
  ...
  // Create the closure, store in cache and return result.
  return __objs2[objId].__closures[funId] = function ()
  {
    return __funs2[funId].apply(__objs2[objId], arguments);
  };
};